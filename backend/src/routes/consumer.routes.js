// Consumer API routes — the entire `/api/*` surface except `/api/admin`
// (its own router). Moved verbatim out of server.js during the backend/src
// restructure: every handler body, validation rule, status code, response
// shape and route-registration order is byte-identical to what lived inline
// in server.js. Each route still declares its full `/api/...` path and
// applies requireAuth per-route, and this router is mounted at the app root
// (not at `/api`), so req.path / req.originalUrl are exactly what they were
// before — requireAuth's 2FA exempt-prefix check relies on that.
...
router.post('/api/me/password-set', requireAuth, ah(async (req, res) => {
...
router.delete('/api/me', requireAuth, ah(async (req, res) => {
...
router.post('/api/me/reset-data', requireAuth, ah(async (req, res) => {
...
router.post('/api/feedback', requireAuth, ah(async (req, res) => {
  const body = req.body || {};
...
router.get('/api/feedback', requireAuth, ah(async (req, res) => {
...
router.get('/api/feedback/:id', requireAuth, ah(async (req, res) => {
...
router.post('/api/feedback/:id/messages', requireAuth, ah(async (req, res) => {
  const item = await adminDb.getFeedback(req.params.id);
  if (!item || item.userId !== req.userId) return res.status(404).json({ error: 'not_found' });
...
router.post('/api/feedback/:id/confirm', requireAuth, ah(async (req, res) => {
  const item = await adminDb.getFeedback(req.params.id);
  if (!item || item.userId !== req.userId) return res.status(404).json({ error: 'not_found' });
...
router.post('/api/login-events', requireAuth, ah(async (req, res) => {
...
router.get('/api/sessions', requireAuth, ah(async (req, res) => {
  const rows = await db.listSessions(req.userId);
  res.json(rows.map((s) => ({ ...s, current: s.sessionId === req.sessionId })));
}));

router.delete('/api/sessions/:id', requireAuth, ah(async (req, res) => {
  const rows = await db.listSessions(req.userId);
...
router.post('/api/2fa/send-code', requireAuth, twoFactorLimiter, ah(async (req, res) => {
...
router.post('/api/2fa/verify', requireAuth, twoFactorLimiter, ah(async (req, res) => {
  const purpose = ['enable', 'login', 'disable'].includes(req.body?.purpose) ? req.body.purpose : 'enable';
...
router.post('/api/impersonation/end', requireAuth, ah(async (req, res) => {
...
router.put('/api/dashboard-layout', requireAuth, ah(async (req, res) => {
...
// ---- categories ----
router.get('/api/categories', requireAuth, (req, res) => sendJSON(req, res, computeCategories(req.userData)));

const CATEGORY_TYPES = ['income', 'expense', 'transfer'];
router.post('/api/categories', requireAuth, ah(async (req, res) => {
...
router.patch('/api/categories/reorder', requireAuth, ah(async (req, res) => {
...
router.delete('/api/categories/:id', requireAuth, ah(async (req, res) => {
  const { id } = req.params;
  const inUse =
...
// ---- accounts ----
router.get('/api/accounts', requireAuth, (req, res) => sendJSON(req, res, computeAccounts(req.userData)));
...
router.delete('/api/accounts/:id', requireAuth, ah(async (req, res) => {
  const { id } = req.params;
...
router.patch('/api/transactions/:id', requireAuth, ah(async (req, res) => {
...
router.post('/api/transactions/bulk', requireAuth, ah(async (req, res) => {
...
router.delete('/api/transactions/:id', requireAuth, ah(async (req, res) => {
...
router.get('/api/budgets', requireAuth, (req, res) => {
...
router.delete('/api/budgets/:id', requireAuth, ah(async (req, res) => {
...
router.get('/api/bills', requireAuth, (req, res) => {
  const { q } = req.query;
...
router.patch('/api/bills/:id', requireAuth, ah(async (req, res) => {
...
router.delete('/api/bills/:id', requireAuth, ah(async (req, res) => {
...
router.get('/api/bills/:id/payments', requireAuth, (req, res) => {
...
router.post('/api/goals', requireAuth, ah(async (req, res) => {
...
router.delete('/api/goals/:id', requireAuth, ah(async (req, res) => {
...
router.post('/api/goals/:id/contribute', requireAuth, ah(async (req, res) => {
  const goal = req.userData.goals.find((g) => g.id === req.params.id);
  if (!goal) return res.status(404).json({ error: 'not found' });
...
// ---- debts ----
router.get('/api/debts', requireAuth, (req, res) => sendJSON(req, res, req.userData.debts));
...
router.patch('/api/debts/:id', requireAuth, ah(async (req, res) => {
...
router.delete('/api/debts/:id', requireAuth, ah(async (req, res) => {
...
router.post('/api/debts/:id/payment', requireAuth, ah(async (req, res) => {
  const debt = req.userData.debts.find((d) => d.id === req.params.id);
  if (!debt) return res.status(404).json({ error: 'not found' });
  const amount = numOr(req.body?.amount);
  if (amount <= 0) return res.status(400).json({ error: 'amount must be greater than 0' });
...
// ---- templates ----
router.get('/api/templates', requireAuth, (req, res) => sendJSON(req, res, req.userData.templates));

router.post('/api/templates', requireAuth, ah(async (req, res) => {
  const b = req.body || {};
...
router.patch('/api/templates/:id', requireAuth, ah(async (req, res) => {
...
router.delete('/api/templates/:id', requireAuth, ah(async (req, res) => {
...
router.get('/api/notifications', requireAuth, ah(async (req, res) => {
...
router.patch('/api/notifications/:id', requireAuth, ah(async (req, res) => {
  await db.upsertNotificationOverlay(req.userId, req.params.id, req.body || {});
  bumpCache(req.userId);
  res.json({ ok: true });
}));

router.post('/api/notifications/read-all', requireAuth, ah(async (req, res) => {
  req.userData.notifications = await db.getNotificationOverlay(req.userId);
  const feedbackTickets = await loadFeedbackNotificationTickets(req.userId);
...
router.delete('/api/notifications/:id', requireAuth, ah(async (req, res) => {
...
// ---- reports ----
router.get('/api/reports', requireAuth, requireFeature('canUseAdvancedAnalytics'), (req, res) => {
...
router.get('/api/ai/insights', requireAuth, requireFeature('canUseAIInsights'), ah(async (req, res) => {
...
router.get('/api/ai/monthly-report', requireAuth, requireFeature('canUseAIReports'), ah(async (req, res) => {
  const usedToday = await db.getAiUsageToday(req.userId);
  if (usedToday >= plans.limitFor(req.userPlan, 'aiRequestsPerDay')) {
    return res.status(403).json({ error: 'upgrade_required', limit: 'aiRequestsPerDay' });
  }
...
router.post('/api/ai/conversations', requireAuth, ah(async (req, res) => {
  const title = req.body && req.body.title;
  const conversation = await db.insertConversation(req.userId, title ? { title } : {});
  res.status(201).json(conversation);
}));

router.get('/api/ai/conversations', requireAuth, ah(async (req, res) => {
...
router.get('/api/ai/conversations/:id/messages', requireAuth, ah(async (req, res) => {
...
router.patch('/api/ai/conversations/:id', requireAuth, ah(async (req, res) => {
...
router.delete('/api/ai/conversations/:id', requireAuth, ah(async (req, res) => {
  await db.deleteConversation(req.userId, req.params.id);
  res.status(204).end();
}));

router.post('/api/ai/conversations/:id/messages', requireAuth, requireFeature('canUseAIInsights'), ah(async (req, res) => {
...
router.post('/api/import', requireAuth, ah(async (req, res) => {
